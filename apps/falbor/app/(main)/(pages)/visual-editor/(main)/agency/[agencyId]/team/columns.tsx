...
import {
  ...
  DropdownMenuTrigger,
} from '~/components/visual-editor/ui/dropdown-menu'
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { deleteUser, getUser } from '~/lib/visual-editor/queries'
import { useToast } from '~/components/visual-editor/ui/use-toast'
import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { UsersWithAgencySubAccountPermissionsSidebarOptions } from '~/lib/visual-editor/types'
  ...
  [
...

    {
      ...
      cell: ({ row }) => {
        ...
        const ownedAccounts = (row.original as any)?.Permissions.filter(
          (per: any) => per.access
        )
            ...
            <div className="flex flex-col items-start">
              <div className="flex flex-col gap-2">
                <Badge className="bg-slate-600 whitespace-nowrap">
                  Agency - {(row?.original as any)?.Agency?.name}
                </Badge>
              </div>
            </div>
          )
...
const CellActions: React.FC<CellActionsProps> = ({ rowData }) => {
  ...
  if (!rowData) return
  if (!(rowData as any).Agency) return
            ...
            onClick={() => {
              setOpen(
                ...
                >
                  <UserDetails
                    type="agency"
                    id={(rowData as any)?.Agency?.id || null}
                    subAccounts={(rowData as any)?.Agency?.SubAccount}
                  />
                </CustomModal>,
                async () => {
