...
import { Progress } from '~/components/visual-editor/ui/progress'
...
const Page = async ({
  ...
  const agencyDetails = await db.query.veAgencies.findFirst({
    where: (agency, { eq }) => eq(agency.id, params.agencyId),
  })

  if (!agencyDetails) return

  const subaccounts = await db.query.veSubAccounts.findMany({
    where: (subaccount, { eq }) => eq(subaccount.agencyId, params.agencyId),
  })
        ...
        <div className="absolute -top-10 -left-10 right-0 bottom-0 z-30 flex items-center justify-center backdrop-blur-md bg-background/50">
          <Card>
            <CardHeader>
              ...
              <Link
                href={`/visual-editor/agency/${agencyDetails.id}/launchpad`}
