...
import CircleProgress from '~/components/visual-editor/global/circle-progress'
import PipelineValue from '~/components/visual-editor/global/pipeline-value'
import SubaccountFunnelChart from '~/components/visual-editor/global/subaccount-funnel-chart'
import { Badge } from '~/components/visual-editor/ui/badge'
import {
  Card,
  CardContent,
  CardDescription,
...
const SubaccountPageId = async ({ params, searchParams }: Props) => {
  ...
  if (subaccountDetails.connectAccountId) {
    const response = await stripe.accounts.retrieve(subaccountDetails.connectAccountId)
  ...
  const allFunnelPages = funnelIds.length > 0
  ...
  const funnelPerformanceMetrics = funnels.map((funnel) => ({
    ...funnel,
    FunnelPages: allFunnelPages.filter((p) => p.funnelId === funnel.id),
    totalFunnelVisits: allFunnelPages
      .filter((p) => p.funnelId === funnel.id)
      .reduce((total, page) => total + page.visits, 0),
  }))
          ...
          <div className="absolute -top-10 -left-10 right-0 bottom-0 z-30 flex items-center justify-center backdrop-blur-md bg-background/50">
            <Card>
              <CardHeader>
                ...
                <Link
                  href={`/visual-editor/subaccount/${subaccountDetails.id}/launchpad`}
