...
const githubUserGET = withSecurity(githubUserLoader, {
  rateLimit: true,
  allowedMethods: ['GET'],
});

export async function GET(request: Request) {
  return githubUserGET({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
...
const githubUserPOST = withSecurity(githubUserAction, {
  rateLimit: true,
  allowedMethods: ['POST'],
});

export async function POST(request: Request) {
  return githubUserPOST({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
