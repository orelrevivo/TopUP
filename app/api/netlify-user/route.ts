...
const netlifyUserGET = withSecurity(netlifyUserLoader, {
  rateLimit: true,
  allowedMethods: ['GET'],
});

export async function GET(request: Request) {
  return netlifyUserGET({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
...
const netlifyUserPOST = withSecurity(netlifyUserAction, {
  rateLimit: true,
  allowedMethods: ['POST'],
});

export async function POST(request: Request) {
  return netlifyUserPOST({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
