...
const vercelUserGET = withSecurity(vercelUserLoader, {
  rateLimit: true,
  allowedMethods: ['GET'],
});

export async function GET(request: Request) {
  return vercelUserGET({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
...
const vercelUserPOST = withSecurity(vercelUserAction, {
  rateLimit: true,
  allowedMethods: ['POST'],
});

export async function POST(request: Request) {
  return vercelUserPOST({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
