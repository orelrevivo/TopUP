...
const supabaseUserGET = withSecurity(supabaseUserLoader, {
  rateLimit: true,
  allowedMethods: ['GET'],
});

export async function GET(request: Request) {
  return supabaseUserGET({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
...
const supabaseUserPOST = withSecurity(supabaseUserAction, {
  rateLimit: true,
  allowedMethods: ['POST'],
});

export async function POST(request: Request) {
  return supabaseUserPOST({ request, context: { cloudflare: { env: process.env as Record<string, string> } } });
}
