...
import { HackingAssistantMessage } from './HackingAssistantMessage';
import { HackingUserMessage } from './HackingUserMessage';
...
export const HackingMessages = forwardRef<HTMLDivElement, MessagesProps>(
  (props: MessagesProps, ref: ForwardedRef<HTMLDivElement> | undefined) => {
    ...
    for (const msg of messages) {
      if (msg.annotations?.includes('hidden')) continue;
        ...
        {groupedMessages.length > 0
          ? groupedMessages.map((message, index) => {
            ...
            return (
              <div
                key={index}
                className={classNames('flex w-full rounded-lg', {
                  'mt-2': !isFirst,
                })}
              >
                <div className="grid grid-col-1 w-full">
                  {isUserMessage ? (
                    <HackingUserMessage content={content} parts={parts} />
                  ) : (
                    <HackingAssistantMessage
                      content={content}
