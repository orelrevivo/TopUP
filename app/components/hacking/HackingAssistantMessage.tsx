...
function parseSkillUsages(content: string) {
  ...
  while ((match2 = regexSelfClosing.exec(content)) !== null) {
    const skillName = match2[1];
    if (!skills.some(s => s.name === skillName)) {
      skills.push({ name: skillName, text: "Skill automatically applied" });
    }
  }
...
function parseBrowserNavigations(content: string) {
  const navigations: string[] = [];
  let cleanContent = content;

  const regex = /<browser_navigate\s+url="([^"]+)"\s*\/>/g;
  let match: RegExpExecArray | null;
  while ((match = regex.exec(content)) !== null) {
    navigations.push(match[1]);
  }

  cleanContent = content.replace(/<browser_navigate\s+url="[^"]+"\s*\/>/g, '');
  return { navigations, cleanContent };
}

function parseInternetSearches(content: string) {
  const searches: { query: string; results: string }[] = [];
  let cleanContent = content;

  const regex = /<internet_search\s+query="([^"]+)">([\s\S]*?)<\/internet_search>/g;
  let match: RegExpExecArray | null;
  while ((match = regex.exec(content)) !== null) {
    searches.push({ query: match[1], results: match[2].trim() });
  }
  ...
  return (
    <div className="tool-invocation border border-falbor-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150 mb-3 bg-falbor-elements-background-depth-1 mt-2">
      <div className="flex">
        ...
        >
          <div className="p-2.5">
            <div className="i-ph:magnifying-glass text-xl text-falbor-elements-textSecondary"></div>
          </div>
            ...
            <div className="px-3 py-3 text-left bg-falbor-elements-background-depth-2">
              <ul className="list-none space-y-4">
                {searches.map((search, index) => (
                  ...
                  >
                    <div className="bg-falbor-elements-background-depth-3 rounded-lg p-3">
                      <div className="font-semibold text-falbor-elements-textPrimary mb-2">
                        Query: {search.query}
                      </div>
                      <div className="mt-2 text-xs text-falbor-elements-textSecondary whitespace-pre-wrap overflow-x-auto">
                        {search.results}
                      </div>
                    </div>
                  </motion.li>
  ...
  ({
    ...
    return (
      <div className="flex flex-col gap-3 items-center w-full">
        <div className="relative flex flex-col bg-accent-500/10 dark:bg-[#252525] backdrop-blur-sm px-5 p-3.5 w-full rounded-lg">
          <>
