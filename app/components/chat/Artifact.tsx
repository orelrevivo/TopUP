...
const ResourcesActionItem = memo(({ action }: { action: any }) => {
  const openResourcesView = () => {
    workbenchStore.currentResourcesData.set(action.content);
    workbenchStore.currentView.set('resources');
    workbenchStore.showWorkbench.set(true);
  };
  ...
  return (
    <div className="flex flex-col w-full border border-falbor-elements-borderColor rounded-md overflow-hidden bg-falbor-elements-background-depth-1 my-2">
      ...
      >
        <div className="flex items-center gap-2">
          <div className="i-ph:link-duotone text-falbor-elements-textSecondary text-lg" />
          <span className="text-sm font-medium text-falbor-elements-textPrimary">
            {action.title || 'Resources & Links'}
          </span>
        </div>
        <div className="text-falbor-elements-textSecondary">
          <div className="i-ph:arrow-right" />
          ...
          if (type === 'resources') {
            return (
              <motion.li
                ...
                className="w-full"
              >
                <ResourcesActionItem action={action} />
              </motion.li>
            );
          }
