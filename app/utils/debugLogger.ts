...
class DebugLogger {
  ...
  private _getCurrentProvider(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('falbor_current_provider');
  ...
  private _getProjectType(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('falbor_project_type');
...
export async function downloadDebugLog(filename?: string): Promise<void> {
  try {
    ...
    link.href = url;
    link.download = filename || `falbor-debug-${new Date().toISOString().split('T')[0]}.txt`;
