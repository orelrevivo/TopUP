    ...
    <div className="min-h-screen bg-falbor-elements-background-depth-1 flex items-start justify-center p-4 sm:p-8 overflow-y-auto">
      <div className="max-w-4xl w-full bg-falbor-elements-background-depth-2 rounded-lg">
        <div className="p-8 md:p-12 flex flex-col gap-6">
          ...
          {report.rawResources && (
            <>
              <div className="h-[1px] w-full bg-falbor-elements-borderColor my-6" />
              <div className="flex items-center gap-2 mb-2">
                <div className="i-ph:link-duotone text-falbor-elements-item-contentAccent text-2xl" />
                <h2 className="text-2xl font-bold text-falbor-elements-textPrimary">Resources & Links</h2>
              </div>
              <ReactMarkdown
                className="flex flex-col gap-6"
                components={{
                  h2: ({ node, ...props }) => (
                    <h2 className="text-xl font-semibold text-falbor-elements-textPrimary mt-6 mb-3 flex items-center gap-2" {...props} />
                  ),
                  h3: ({ node, ...props }) => (
                    <h3 className="text-lg font-medium text-falbor-elements-textPrimary mt-4 mb-2" {...props} />
                  ),
                  p: ({ node, ...props }) => (
                    <p className="text-base text-falbor-elements-textSecondary leading-relaxed m-0" {...props} />
                  ),
